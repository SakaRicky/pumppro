...
import { DailySaleSummary, SaleWithDetails } from "../src/types";
...
const saveFuelsSale = async (
	fuelsToSell: FuelToSell[]
): Promise<FuelSale[]> => {
...
const saveDailySalesSummary = async (
	moneyNotGiven: Decimal,
	productsSold?: SaleWithDetails[]
) => {
	let newDailySaleSummary: DailySaleSummary;
	// a seller can sell both at the shop and at the pump

	if (productsSold) {
		// Calculate expected and given amounts
		const expectedMoneyFromSeller = productsSold
			?.map(s => s.sale_details)
			.flat()
			.reduce((acc, item) => {
		...
		newDailySaleSummary = await prisma.dailySalesSummary.create({
			data: {
...
export async function seed() {
	try {
		...
		const initialProductsInDB = await createProducts(
			initialProductCategoriesInDB
		);

		const productsToSell = [
			initialProductsInDB[0],
			initialProductsInDB[1],
			initialProductsInDB[3],
			initialProductsInDB[3],
			initialProductsInDB[4],
			initialProductsInDB[5],
			initialProductsInDB[6],
			initialProductsInDB[7]
		];

		const dailySaleData = [
			{
				user: users[0],
				productsSold: productsToSell.slice(0, 3),
				quantitySold: [2, 1, 14],
				moneyNotGiven: new Decimal(0)
			},
			{
				user: users[3],
				productsSold: productsToSell.slice(3),
				quantitySold: [4, 7, 2, 6, 1],
				moneyNotGiven: new Decimal(200)
			}
		];

		const initialProductsSold = await Promise.all(
			dailySaleData.map(d =>
				saveProductsSale(d.user, d.productsSold, d.quantitySold)
			)
		);
