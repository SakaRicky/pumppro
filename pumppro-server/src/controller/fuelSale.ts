...
import { validateNewFuelSale, validateExistingFuelSale } from "../utils/validateData";
...
const getAllFuels = async (where: any): Promise<FuelSale[]> => {
...
export const getFuelSales = async (
    ...
    const { startDate, stopDate, userID } =
...
export const saveFuelSale = async (req: RequestWithToken, res: Response) => {
    ...
    const fuel = await prisma.fuel.findUnique({ where: { id: newFuelSale.fuel_id } });
    ...
    await prisma.fuelSale.create({
        data: {
            ...
            total_amount: fuel.selling_price.mul(newFuelSale.end_reading.minus(newFuelSale.start_reading))
        }
    });
...
export const updateFuelSale = async (req: RequestWithToken, res: Response) => {
    ...
    const fuel = await prisma.fuel.findUnique({ where: { id: existingFuelSale.fuel_id } });
    ...
    await prisma.fuelSale.update({
        data: {
            ...
            quantity_sold: existingFuelSale.end_reading.minus(existingFuelSale.start_reading),
            total_amount: fuel.selling_price.mul(existingFuelSale.end_reading.minus(existingFuelSale.start_reading))
        },
...
export const deleteFuelSale = async (req: RequestWithToken, res: Response) => {
    const body = req.body;
...
export const deleteManyFuelSale = async (req: RequestWithToken, res: Response) => {
    const body = req.body;
    const fuelSalesToDelete = body.ids;
