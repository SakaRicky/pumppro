...
interface ProductWithCategoryArgs {
	include: { category: true };
}

export type ProductWithCategory =
	Prisma.ProductGetPayload<ProductWithCategoryArgs>;
...
export const getSalesSummary = async (
	...
	try {
		...
		const relevantSaleDetails = allSalesForPeriod
			...
			.filter(
				detail =>
					...
					!selectedCategoryID ||
					detail.product.category_id === selectedCategoryID
			);

		// 4. Aggregate Sales per Product using a Map
		const productSalesAggregates = new Map<
			string,
			{
				quantity: number;
				total_amount: Decimal;
		...
		for (const detail of relevantSaleDetails) {
			...
			if (existing) {
				// update what's already in the productSalesAggregates
				existing.quantity += detail.quantity;
				existing.total_amount = existing.total_amount.add(detailAmount);
			} else {
				//populate the productSalesAggregates
		...
		const allProductsInScope = await prisma.product.findMany({
			where: selectedCategoryID ? { category_id: selectedCategoryID } : {} // return all products if no category was provided
		});
		...
		const salesSummary = allProductsInScope.map(product => {
			...
			if (aggregate) {
				totalAmountSoldForThisPeriodInThisCategory =
					totalAmountSoldForThisPeriodInThisCategory.add(amount);
				benefitsForThisPeriodInThisCategory =
					benefitsForThisPeriodInThisCategory.add(profitForItem);
		...
		return res.status(200).send({
			...
			totalAmountSoldForThisPeriodInThisCategory:
				totalAmountSoldForThisPeriodInThisCategory.toNumber(),
			totalAmountSoldAllCategories: totalAmountSoldAllCategories.toNumber(),
			benefitsForThisPeriodInThisCategory:
				benefitsForThisPeriodInThisCategory.toNumber()
		});
	} catch (error) {
