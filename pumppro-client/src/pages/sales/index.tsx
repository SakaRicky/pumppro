...
const Sales = () => {
  ...
  const MobileSaleCard = ({ sale }: { sale: Sale }) => {
    return (
      <Box
        sx={{ boxShadow: 2, my: 2, p: 2, borderRadius: "0.25rem" }}
      ...
      >
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <Box>
            ...
            <Box>{moment(sale.created_at).format("DD/MM/YYYY")} </Box>
          </Box>
          <Box>
            <Typography
  ...
  return isMobile ? (
    ...
    <Box sx={{ p: 2, display: "flex", gap: "2rem" }}>
      <Box sx={{ width: "55%", height: "60vh" }}>
        ...
        {!isLoading && !data && (
          <Typography>Loading data failed or data is unavailable.</Typography>
        )}
